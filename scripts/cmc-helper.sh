...
# Usage: ./cmc-helper.sh [command] [options]

set -e

CMC_CMC_WORKFLOW_DIR="$HOME/.claude-master-and-commander"
...
print_usage() {
    ...
    echo "  parse <cmc-file>       Parse PRD and generate task breakdown"
...
create_new_project() {
    ...
    echo -e "${BLUE}Creating new CMC project: $project_name${NC}"
    
    # Create project structure
    mkdir -p "$project_dir"/{docs,src,tests,scripts,assets}
    
    # Copy PRD template
    cp "$CMC_WORKFLOW_DIR/templates/prd-template.md" "$project_dir/docs/prd.md"
...
EOF
    ...
    echo "2. Run: cmc-generate $project_dir/docs/prd.md"
...
parse_cmc() {
    local cmc_file="$1"
    if [[ ! -f "$cmc_file" ]]; then
        echo -e "${RED}Error: PRD file not found: $cmc_file${NC}"
        exit 1
    fi
    
    local project_dir=$(dirname $(dirname "$cmc_file"))
    ...
    local requirements=$(grep -A 10 "## Functional Requirements" "$cmc_file" | grep -E "^\s*\*|\s*-|\d\." || true)
...
show_project_status() {
    ...
    if [[ ! -f "$config_file" ]]; then
        echo -e "${RED}Error: Not a CMC project (missing .cmc-config.json)${NC}"
        exit 1
    fi
...
list_projects() {
    echo -e "${BLUE}CMC Projects:${NC}"
    echo
    
    find "$PROJECTS_DIR" -name ".cmc-config.json" -type f 2>/dev/null | while read config_file; do

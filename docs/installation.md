...
alias cmc="$WORKFLOW_DIR/scripts/cmc-helper.sh"
alias cmc-new="$WORKFLOW_DIR/scripts/cmc-helper.sh new"
alias cmc-parse="$WORKFLOW_DIR/scripts/cmc-helper.sh parse"
alias cmc-status="$WORKFLOW_DIR/scripts/cmc-helper.sh status"
alias cmc-list="$WORKFLOW_DIR/scripts/cmc-helper.sh list"

# Enhanced task generation
cmc-generate() {
    if [[ -z "$1" ]]; then
        echo "Usage: cmc-generate <path-to-prd.md>"
        return 1
    fi
    node "$WORKFLOW_DIR/scripts/cmc-task-generator.js" "$1"
}
...
cmc-cd() {
    ...
    if [[ -z "$project" ]]; then
        ...
        "$WORKFLOW_DIR/scripts/cmc-helper.sh" list
        return 1
    fi
    ...
    if [[ -d "$project_path" ]]; then
        ...
        if [[ -f ".cmc-config.json" ]]; then
            "$WORKFLOW_DIR/scripts/cmc-helper.sh" status .
        fi
    else
...
- **Issues**: [GitHub Issues](https://github.com/jparkypark/claude-master-and-commander/issues)
- **Questions**: [GitHub Discussions](https://github.com/jparkypark/claude-master-and-commander/discussions)
